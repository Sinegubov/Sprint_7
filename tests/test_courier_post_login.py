...
class TestPostCourierLogin:
    ...
    def test_courier_login(self, generator,
                           url_create=URL.COURIER_URL,
                           url=URL.COURIER_LOGIN_URL):
        payload = generator.register_new_courier_and_return_login_password()
        requests.post(url_create, data=payload)
        response = requests.post(url, data=payload)
        assert response.status_code == 200
        assert response.json()["id"] != ""
        generator.delete_courier(payload["login"], payload["password"])

    @allure.title('Авторизация несуществующим пользователе возвращает ошибку и код 404')
    def test_courier_not_valid_login_negative(self, generator, url=URL.COURIER_LOGIN_URL):
